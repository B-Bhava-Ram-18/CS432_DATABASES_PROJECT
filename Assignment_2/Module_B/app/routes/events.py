...
def list_events():
    db = get_db()
    rows = db.execute(
        "SELECT Event_ID, Event_Name, Start_Time, End_Time, Status FROM Event ORDER BY Start_Time DESC"
    ).fetchall()
    db.close()
...
def create_event():
    ...
    for f in ["Event_ID", "Event_Name", "Start_Time", "End_Time", "Status"]:
        if f not in data:
            return jsonify({"error": f"Missing: {f}"}), 400
    db = get_db()
    try:
        db.execute(
            "INSERT INTO Event (Event_ID, Event_Name, Start_Time, End_Time, Status) VALUES (?,?,?,?,?)",
            (data["Event_ID"], data["Event_Name"], data["Start_Time"], data["End_Time"], data["Status"])
        )
        db.commit()
...
def update_event(event_id):
    data = request.get_json(silent=True) or {}
    allowed = ["Status", "Event_Name", "Start_Time", "End_Time"]
