...
def list_stats():
    ...
    if user["role"] == "admin":
        rows = db.execute(
            """SELECT ps.*, m.Name FROM Performance ps JOIN Member m ON ps.Member_ID=m.Member_ID
               ORDER BY ps.Recorded_Date DESC"""
        ).fetchall()
    else:
        rows = db.execute(
            "SELECT * FROM Performance WHERE Member_ID=? ORDER BY Recorded_Date DESC",
            (user["member_id"],)
        ).fetchall()
    db.close()
...
def create_stat():
    ...
    try:
        db.execute(
            "INSERT INTO Performance (Member_ID, Event_ID, Metric_Name, Metric_Value, Recorded_Date) VALUES (?,?,?,?,?)",
        ...
        log_action(request.current_user["username"], request.current_user["member_id"],
                   "admin", "CREATE", "Performance", data["Member_ID"],
...
def update_stat(member_id, metric_name, recorded_date):
    ...
    try:
        db.execute(
            "UPDATE Performance SET Metric_Value=? WHERE Member_ID=? AND Metric_Name=? AND Recorded_Date=?",
            (data["Metric_Value"], member_id, metric_name, recorded_date)
        )
        db.commit()
        log_action(request.current_user["username"], request.current_user["member_id"],
                   "admin", "UPDATE", "Performance", member_id,
...
def delete_stat(member_id, metric_name, recorded_date):
    ...
    try:
        result = db.execute(
            "DELETE FROM Performance WHERE Member_ID=? AND Metric_Name=? AND Recorded_Date=?",
        ...
        log_action(request.current_user["username"], request.current_user["member_id"],
                   "admin", "DELETE", "Performance", member_id,
