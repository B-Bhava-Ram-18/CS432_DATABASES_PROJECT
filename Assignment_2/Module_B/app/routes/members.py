...
def portfolio(member_id):
    ...
    player = db.execute(
        "SELECT Member_ID,Player_ID,Height,Weight,Blood_Group FROM Player WHERE Member_ID=?",
        (member_id,)
    ).fetchone()
    coach = db.execute(
    ...
    if player:
        stats = [dict(r) for r in db.execute(
            "SELECT Metric_Name,Metric_Value,Recorded_Date,Event_ID FROM Performance WHERE Member_ID=? ORDER BY Recorded_Date DESC",
            (member_id,)
        ).fetchall()]
        teams = [dict(r) for r in db.execute(
            """SELECT t.Team_Name, t.Category, s.Sport_Name
               FROM Team_Roster tr
               JOIN Player p ON tr.Member_ID=p.Member_ID
