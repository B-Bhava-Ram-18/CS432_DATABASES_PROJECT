...
def list_players():
    db = get_db()
    rows = db.execute(
        """SELECT p.Member_ID, p.Player_ID, p.Height, p.Weight, p.Blood_Group,
...
def create_player():
    ...
    for f in ["Member_ID", "Player_ID"]:
        if f not in data:
            return jsonify({"error": f"Missing: {f}"}), 400
    db = get_db()
    try:
        db.execute(
            "INSERT INTO Player (Member_ID, Player_ID, Height, Weight, Blood_Group) VALUES (?,?,?,?,?)",
            (data["Member_ID"], data["Player_ID"],
