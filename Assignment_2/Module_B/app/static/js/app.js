    ...
    <div class="portfolio-hero">
      ...
      <div>
        ...
        <div class="portfolio-meta">
          ...
          ${p.player_info?badge("Player #"+p.player_info.Player_ID,"green"):""}
...
async function renderPlayers(c) {
  ...
  <div class="table-wrap"><table>
    <thead><tr><th>Member ID</th><th>Player ID</th><th>Name</th><th>Gender</th><th>Age</th><th>Height</th><th>Weight</th><th>Blood Group</th><th>Email</th>${isAdmin?"<th>Actions</th>":""}</tr></thead>
    <tbody>${rows.map(r=>`<tr>
      <td>${mono(r.Member_ID)}</td>
      <td>${mono(r.Player_ID)}</td>
...
function openAddPlayer() {
  openModal(`
    ...
    <div class="modal-body">
      ...
      <div class="form-group"><label>Player ID</label><input id="ap-roll" type="number" placeholder="121"/></div>
...
async function submitPlayer() {
  try {
    await API.createPlayer({
      ...
      Player_ID: parseInt($("ap-roll").value),
...
async function openRoster(teamId, teamName) {
  ...
  openModal(`
    ...
    <div class="modal-body">
      <div class="table-wrap"><table>
        <thead><tr><th>Player ID</th><th>Member ID</th><th>Name</th></tr></thead>
        <tbody>${rows.map(r=>`<tr><td>${mono(r.Player_ID)}</td><td>${mono(r.Member_ID)}</td><td>${r.Name}</td></tr>`).join("")}</tbody>
      </table></div>
    </div>`);
}
...
async function renderFacilities(c) {
  ...
  <div class="table-wrap"><table>
    ...
    <tbody>${rows.map(r=>`<tr>
      ...
      <td style="color:var(--text3);font-size:12px">${r.Description}</td>
...
async function renderEvents(c) {
  ...
  <div class="table-wrap"><table>
    <thead><tr><th>ID</th><th>Name</th><th>Start</th><th>End</th><th>Status</th>${isAdmin?"<th>Actions</th>":""}</tr></thead>
    <tbody>${rows.map(r=>`<tr>
      ...
      <td>${badge(r.Status, r.Status==="Completed"?"green":r.Status==="Cancelled"?"red":"yellow")}</td>
      ${isAdmin?`<td><div class="flex gap-2">
        <button class="btn btn-ghost btn-sm" onclick="openEditEvent('${r.Event_ID}','${r.Status}')">Edit</button>
...
async function submitEditEvent(id) {
  try { await API.updateEvent(id,{Status:$("ev-st").value}); closeModal(); navigate("events"); }
  catch(e) { alert(e.message); }
}
...
async function submitAddEvent() {
  try {
    await API.createEvent({Event_ID:$("ae-id").value.trim(),Event_Name:$("ae-name").value.trim(),
      Start_Time:$("ae-start").value.trim(),End_Time:$("ae-end").value.trim(),Status:$("ae-st").value});
